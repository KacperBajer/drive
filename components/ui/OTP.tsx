  ...
  return (
    <div className='fixed top-0 left-0 h-screen w-full flex justify-center items-center bg-black/50'>
        <div className='h-full w-full sm:h-fit sm:w-fit bg-white sm:rounded-xl relative flex flex-col px-5 py-8'>
            ...
            <p className='font-semibold text-3xl text-center text-dark-200 mb-3'>Enter OTP</p>
            <p className='font-medium text-dark-100 text-lg text-center'>We've send a code to <span className='text-red-100'>{email}</span></p>
            <div className='my-7 flex gap-2'>
              <OTPForm handleSubmit={handleSubmit} />
            </div>
            <p className='font-medium text-dark-100 text-lg text-center'>Didn't get a code? <span className='text-red-100 hover:cursor-pointer'>Click to resend.</span></p>
        </div>
    </div>
  )
}
